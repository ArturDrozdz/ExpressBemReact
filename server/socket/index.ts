...
module.exports = function (server: Server) {
    ...
    io.set('authorization',
        function (handshake: _Handshake, callback: (e: Event | Error | null, check?: boolean) => void) {
            async.waterfall([
    ...
    io.sockets.on('connection', function (socket: Socket) {
        const username = socket.client.request.user.nick;
        socket.broadcast.emit('join', username);
        socket.on('message', function (text: string, cb: () => void) {
